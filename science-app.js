// UPSC Science & Technology Tracker - Application Logic

class ScienceTracker {
    constructor() {
    ...
    loadNotes() {
        const saved = localStorage.getItem('scienceNotes');
        return saved ? JSON.parse(saved) : {};
    }
    saveNotes() {
        localStorage.setItem('scienceNotes', JSON.stringify(this.notes));
    }
    loadAttempted() {
        const saved = localStorage.getItem('scienceAttempted');
        return saved ? JSON.parse(saved) : {};
    }
    saveAttempted() {
        localStorage.setItem('scienceAttempted', JSON.stringify(this.attemptedQuestions));
    }
    ...
    getWrongQuestions() {
        ...
        Object.entries(this.attemptedQuestions).forEach(([key, val]) => {
            if (!val.correct) {
                ...
                const topic = scienceData.microthemes.find(t => t.id === topicId);
    ...
    renderTopicList() {
        ...
        const mainsCount = typeof scienceMainsQuestions !== 'undefined' ? scienceMainsQuestions.length : 0;
        ...
        topicList.innerHTML = `
            ...
            ${scienceData.microthemes.map(topic => `
    ...
    selectTopic(topicId) {
        ...
        if (topicId === 'all') {
            ...
            this.currentTopic = scienceData.microthemes.find(t => t.id === topicId);
    ...
    showQuizSetup() {
        ...
        const totalAvailable = scienceQuestions.length;
    ...
    getUnattemptedGlobal() {
        const result = [];
        scienceData.microthemes.forEach(topic => {
    ...
    renderAnalytics() {
        ...
        const themeStats = scienceData.microthemes.map(topic => {
    ...
    renderSyllabus() {
        ...
        c.innerHTML = `
            <div class="syllabus-section">
                <h3>S&T Syllabus Coverage</h3>
                <ul>${scienceData.syllabus.map(i=>`<li>${i}</li>`).join('')}</ul>
            </div>
            <div class="syllabus-section">
    ...
    renderTopicMains() {
        ...
        const themeMap = {
            ...
            'Astrophysics & Space Technology': ['Space & Defence'],
            'Biotechnology': ['Biotechnology', 'Nanotechnology'],
            'Physics': ['Nuclear & Alternative Energy'],
            'Chemistry': ['Climate & Environment'],
            'Biology': ['Biotechnology', 'Health & Pharma'],
            'Diseases': ['Health & Pharma']
        };
        const mainsThemes = themeMap[this.currentTopic.name] || [];
        const relevant = typeof scienceMainsQuestions !== 'undefined'
            ? scienceMainsQuestions.filter(q => mainsThemes.includes(q.theme))
            : [];
    ...
    renderAllMains() {
        const c = document.getElementById('mainsContent');
        const allQ = typeof scienceMainsQuestions !== 'undefined' ? scienceMainsQuestions : [];
        ...
        c.innerHTML = `
            <div style="margin-bottom:25px;">
                ...
                <p style="color:var(--text-muted);margin-bottom:20px;">UPSC Mains GS Paper 3 Science & Technology (2013-2024)</p>
    ...
    renderFilteredMains(yearFilter='all', themeFilter='all') {
        let filtered = typeof scienceMainsQuestions !== 'undefined' ? scienceMainsQuestions : [];
    ...
    updateStats() {
        const totalPrelims = scienceQuestions.length;
        ...
        document.getElementById('mainsCount').textContent = typeof scienceMainsQuestions !== 'undefined' ? scienceMainsQuestions.length : 0;
...
document.addEventListener('DOMContentLoaded', () => {
    window.scienceTracker = new ScienceTracker();
    setTimeout(() => window.scienceTracker.selectTopic('all'), 100);
});
