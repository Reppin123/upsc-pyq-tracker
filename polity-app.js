// UPSC Polity Tracker - Application Logic (Gen 2)

class PolityTracker {
    constructor() {
        this.currentTopic = null;
        this.currentView = null; // 'wrong', 'analytics', 'quiz', or null
    ...
    loadNotes() {
        const saved = localStorage.getItem('polityNotes');
        return saved ? JSON.parse(saved) : {};
    }
    saveNotes() {
        localStorage.setItem('polityNotes', JSON.stringify(this.notes));
    }
    loadAttempted() {
        const saved = localStorage.getItem('polityAttempted');
        return saved ? JSON.parse(saved) : {};
    }
    saveAttempted() {
        localStorage.setItem('polityAttempted', JSON.stringify(this.attemptedQuestions));
    }
    ...
    getWrongQuestions() {
        ...
        Object.entries(this.attemptedQuestions).forEach(([key, val]) => {
            if (!val.correct) {
                ...
                const topic = polityData.microthemes.find(t => t.id === topicId);
    ...
    renderTopicList() {
        ...
        const mainsCount = polityData.allMainsQuestions ? polityData.allMainsQuestions.length : 0;
        ...
        topicList.innerHTML = `
            ...
            ${polityData.microthemes.map(topic => `
    ...
    selectTopic(topicId) {
        ...
        if (topicId === 'all') {
            ...
            this.currentTopic = polityData.microthemes.find(t => t.id === topicId);
            ...
            <div class="question-card wrong-card" style="border-left:4px solid var(--danger);">
                ...
                <div class="answer-section" style="display:block;">
                    <div style="margin-bottom:6px;">
                        <span class="wrong-selected">Your answer: (${w.selected}) ${q.options[w.selected] || 'N/A'}</span>
                    </div>
    ...
    showQuizSetup() {
        ...
        const totalAvailable = polityQuestions.length;
    ...
    getUnattemptedGlobal() {
        const result = [];
        polityData.microthemes.forEach(topic => {
    ...
    renderAnalytics() {
        ...
        const themeStats = polityData.microthemes.map(topic => {
    ...
    // ==========================================
    // EXISTING FEATURES (upgraded to Gen 2)
    // ==========================================
    renderSyllabus() {
        ...
        c.innerHTML = `
            <div class="syllabus-section">
                ...
                <ul>${polityData.syllabus.prelims.slice(0,10).map(i=>`<li>${i}</li>`).join('')}</ul>
            </div>
            <div class="syllabus-section">
                <h3>Mains Syllabus Coverage</h3>
                <ul>${polityData.syllabus.mains.slice(0,10).map(i=>`<li>${i}</li>`).join('')}</ul>
            </div>
            <div class="syllabus-section">
    ...
    renderTopicMains() {
        ...
        const relevant = polityData.allMainsQuestions.filter(q => {
        ...
        this.attachMainsToggleHandlers(c);
    }

    isTopicRelevant(micro, tag) {
        const map = {
            'political theory': ['constitutional philosophy', 'democracy', 'separation of powers'],
            'fundamental rights and duties': ['article 14', 'article 19', 'article 21', 'right to equality', 'freedom of speech', 'fundamental rights'],
            'judiciary': ['supreme court', 'judicial review', 'judicial activism', 'njac', 'appointment of judges'],
            'dpsp': ['directive principles'],
            'parliament procedures': ['legislature', 'lok sabha', 'rajya sabha', 'parliamentary'],
            'governor': ['state executive'],
            'federalism': ['centre-state', 'cooperative federalism'],
            'local self-government': ['panchayati raj', 'local governance', '73rd amendment', '74th amendment'],
            'rpa, elections': ['election commission', 'rpa 1951', 'electoral'],
            'constitutional bodies': ['cag', 'finance commission', 'election commission'],
            'president powers': ['executive', 'pardoning power'],
            'amendment': ['constitutional amendments', 'article 368', 'basic structure'],
            'emergency': ['emergency provisions', 'article 356', 'financial emergency'],
            'parliament committees': ['standing committees', 'parliamentary committees'],
            'parliament houses': ['rajya sabha', 'lok sabha', 'joint session'],
            'political parties': ['party system', 'anti-defection'],
            'preamble': ['preamble', 'constitutional philosophy']
        };
    ...
    renderAllMains() {
        const c = document.getElementById('mainsContent');
        const allQ = polityData.allMainsQuestions;
        ...
        c.innerHTML = `
            <div style="margin-bottom:25px;">
                ...
                <p style="color:var(--text-muted);margin-bottom:20px;">UPSC Mains GS Paper 2 Polity Questions (2013-2025)</p>
    ...
    filterMains() {
        ...
        container.innerHTML = this.renderFilteredMains(yf, tf);
        this.attachMainsToggleHandlers(container);
    }

    renderFilteredMains(yearFilter='all', topicFilter='all') {
        let filtered = polityData.allMainsQuestions;
    ...
    renderMainsCard(q, idx) {
        return `
            <div class="mains-question">
                ...
                ${q.modelAnswer ? `
                    <button class="toggle-answer" data-idx="${idx}">Show Model Answer</button>
                    <div class="model-answer" id="mains-answer-${idx}">
                        <div style="color:var(--success);font-weight:bold;margin-bottom:10px;">Model Answer:</div>
                        <div style="color:var(--text);line-height:1.8;white-space:pre-line;">${q.modelAnswer}</div>
                    </div>
                ` : ''}
            </div>`;
    }

    attachMainsToggleHandlers(container) {
        (container || document).querySelectorAll('.toggle-answer[data-idx]').forEach(btn => {
            btn.addEventListener('click', () => {
                const idx = btn.dataset.idx;
                const modelAnswer = document.getElementById(`mains-answer-${idx}`);
                if (modelAnswer.classList.contains('show')) {
                    modelAnswer.classList.remove('show');
    ...
    updateStats() {
        const totalPrelims = polityQuestions.length;
        ...
        document.getElementById('mainsCount').textContent = polityData.allMainsQuestions ? polityData.allMainsQuestions.length : 0;
...
document.addEventListener('DOMContentLoaded', () => {
    window.polityTracker = new PolityTracker();
    setTimeout(() => window.polityTracker.selectTopic('all'), 100);
});
