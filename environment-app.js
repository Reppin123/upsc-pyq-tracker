// UPSC Environment & Ecology Tracker - Application Logic
...
class EnvironmentTracker {
    ...
    loadNotes() {
        const saved = localStorage.getItem('environmentNotes');
        return saved ? JSON.parse(saved) : {};
    }
    saveNotes() {
        localStorage.setItem('environmentNotes', JSON.stringify(this.notes));
    }
    loadAttempted() {
        const saved = localStorage.getItem('environmentAttempted');
        return saved ? JSON.parse(saved) : {};
    }
    saveAttempted() {
        localStorage.setItem('environmentAttempted', JSON.stringify(this.attemptedQuestions));
    }
    ...
    getWrongQuestions() {
        ...
        Object.entries(this.attemptedQuestions).forEach(([key, val]) => {
            if (!val.correct) {
                ...
                const topic = environmentData.microthemes.find(t => t.id === topicId);
    ...
    renderTopicList() {
        ...
        const mainsCount = typeof environmentMainsQuestions !== 'undefined' ? environmentMainsQuestions.length : 0;
        ...
        topicList.innerHTML = `
            ...
            ${environmentData.microthemes.map(topic => `
                <div class="topic-item" data-topic-id="${topic.id}">
                    <span>${topic.icon} ${topic.name}</span>
                    <span class="topic-badge">${topic.questionCount}</span>
                </div>
            `).join('')}
        `;
    }
    ...
    selectTopic(topicId) {
        ...
        if (topicId === 'all') {
            ...
            this.currentTopic = environmentData.microthemes.find(t => t.id === topicId);
            document.getElementById('topicTitle').textContent = `${this.currentTopic.icon} ${this.currentTopic.name}`;
            ...
            <div class="question-card wrong-card" style="border-left:4px solid var(--danger);">
                <div class="question-header">
                    <div>
                        ...
                        ${q.subtheme ? `<span class="subtheme-tag">${q.subtheme}</span>` : ''}
                    </div>
                ...
                <div class="options">
                    ${Object.entries(q.options).map(([key, val]) => {
                        ...
                        return `<div class="option ${cls}" style="pointer-events:none;">(${key.toUpperCase()}) ${val}</div>`;
                    }).join('')}
                </div>
                <div class="answer-section" style="display:block;">
                    <div style="margin-bottom:6px;">
                        <span class="wrong-selected">Your answer: (${w.selected ? w.selected.toUpperCase() : 'Timed out'}) ${w.selected ? q.options[w.selected] : ''}</span>
                    </div>
                    <div class="answer-label">Correct: (${q.answer.toUpperCase()}) ${q.options[q.answer]}</div>
    ...
    showQuizSetup() {
        ...
        const totalAvailable = environmentQuestions.length;
    ...
    getUnattemptedGlobal() {
        const result = [];
        environmentData.microthemes.forEach(topic => {
    ...
    renderQuizQuestion() {
            ...
            <div class="quiz-container">
                <div class="quiz-header">
                    <div>
                        ...
                        <div class="quiz-progress-text">${item.topicName} | ${q.year}${q.subtheme ? ' | ' + q.subtheme : ''}</div>
                    </div>
                ...
                <div class="quiz-body">
                    ...
                    <div id="qzOptions">
                        ${Object.entries(q.options).map(([key, val]) =>
                            `<div class="quiz-option" data-key="${key}">(${key.toUpperCase()}) ${val}</div>`
                        ).join('')}
                    </div>
        ...
        document.getElementById('qzQuit').addEventListener('click', () => this.endQuiz());
    ...
    handleQuizAnswer(selectedKey) {
        ...
        if (isCorrect) {
            ...
            feedback.innerHTML = `<div style="color:var(--danger);font-weight:600;margin-bottom:8px;">Wrong! Correct answer: (${q.answer.toUpperCase()})</div>`;
        }
        if (q.explanation) {
    ...
    handleQuizTimeout() {
        ...
        const feedback = document.getElementById('qzFeedback');
        feedback.style.display = 'block';
        feedback.innerHTML = `<div style="color:var(--danger);font-weight:600;margin-bottom:8px;">Time's Up! Correct answer: (${q.answer.toUpperCase()})</div>`;
        if (q.explanation) {
            feedback.innerHTML += `<div class="explanation-text">${q.explanation}</div>`;
        }
    ...
    showQuizResults() {
        ...
        qs.answers.forEach(a => {
            if (!byTheme[a.topicName]) byTheme[a.topicName] = { correct: 0, total: 0 };
            byTheme[a.topicName].total++;
            if (a.correct) byTheme[a.topicName].correct++;
        });
        ...
        overlay.innerHTML = `
            <div class="quiz-container">
                <div class="quiz-header"><h3>Quiz Complete!</h3></div>
        ...
        document.getElementById('qzClose').addEventListener('click', () => { overlay.remove(); this.quizState = null; });
        document.getElementById('qzRetry').addEventListener('click', () => { overlay.remove(); this.quizState = null; this.selectSpecialView('quiz'); });
    }
    ...
    renderAnalytics() {
        ...
        const themeStats = environmentData.microthemes.map(topic => {
        ...
        c.innerHTML = `
            <div class="analytics-summary">
                <div class="summary-item"><div class="num">${totalAttempted}</div><div class="lbl">Attempted</div></div>
                <div class="summary-item"><div class="num" style="color:var(--success);">${totalCorrect}</div><div class="lbl">Correct</div></div>
                <div class="summary-item"><div class="num" style="color:var(--danger);">${totalWrong}</div><div class="lbl">Wrong</div></div>
                <div class="summary-item"><div class="num">${accuracy}%</div><div class="lbl">Accuracy</div></div>
            </div>
            <div class="analytics-grid">
                    ...
                    <div class="bar-chart">
                        ${themeStats.map(t => {
                            ...
                            return `<div class="bar-row">
                                <div class="bar-label" title="${t.name}">${t.icon} ${t.name}</div>
                                <div class="bar-track"><div class="bar-fill ${cls}" style="width:${pct}%"></div><div class="bar-value">${label}</div></div>
                            </div>`;
                        }).join('')}
                    </div>
                </div>
                <div class="analytics-card">
                    <h4>Strong Areas</h4>
                    ${strong.length > 0 ? strong.map(t => `<span class="strength-tag strong">${t.icon} ${t.name} (${t.accuracy}%)</span>`).join('') : '<p style="color:var(--text-muted);font-size:0.9em;">Attempt at least 3 questions per theme to see strength analysis</p>'}
                </div>
                <div class="analytics-card">
                    <h4>Weak Areas - Focus Here</h4>
                    ${weak.length > 0 ? weak.map(t => `<span class="strength-tag weak">${t.icon} ${t.name} (${t.accuracy}%)</span>`).join('') : '<p style="color:var(--text-muted);font-size:0.9em;">No weak areas identified yet. Keep practicing!</p>'}
                </div>
                <div class="analytics-card">
                    ...
                    <div class="bar-chart">
                        ${themeStats.map(t => {
                            ...
                            return `<div class="bar-row">
                                <div class="bar-label" title="${t.name}">${t.icon} ${t.name}</div>
                                <div class="bar-track"><div class="bar-fill ${pct >= 80 ? 'strong' : pct >= 40 ? 'moderate' : 'neutral'}" style="width:${pct}%"></div><div class="bar-value">${t.attempted}/${t.total}</div></div>
                            </div>`;
                        }).join('')}
                    </div>
                </div>
                <div class="analytics-card">
                    ...
                    ${Object.keys(yearStats).length > 0 ? `<div class="bar-chart">
                        ${Object.entries(yearStats).sort((a,b) => b[0]-a[0]).map(([year, data]) => {
                            const pct = Math.round((data.correct / data.total) * 100);
                            const cls = pct >= 70 ? 'strong' : pct >= 40 ? 'moderate' : 'weak';
                            return `<div class="bar-row"><div class="bar-label">${year}</div><div class="bar-track"><div class="bar-fill ${cls}" style="width:${pct}%"></div><div class="bar-value">${pct}% (${data.correct}/${data.total})</div></div></div>`;
                        }).join('')}
                    </div>` : '<p style="color:var(--text-muted);font-size:0.9em;">No data yet</p>'}
                </div>
            </div>
            ${notStarted.length > 0 ? `<div class="analytics-card" style="margin-top:15px;">
                <h4>Not Yet Started</h4>
                <p style="color:var(--text-muted);font-size:0.9em;margin-bottom:10px;">These themes have no attempts yet:</p>
                ${notStarted.map(t => `<span class="strength-tag weak" style="background:var(--surface-light);color:var(--text-muted);">${t.icon} ${t.name} (${t.total} Qs)</span>`).join('')}
            </div>` : ''}`;
    }
    ...
    renderSyllabus() {
        ...
        const total = this.currentTopic.questionCount;
        const pct = total > 0 ? Math.round((attempted/total)*100) : 0;
        const subthemes = [...new Set(this.currentTopic.questions.map(q => q.subtheme))];

        c.innerHTML = `
            <div class="syllabus-section">
                <h3>Environment & Ecology Syllabus Coverage</h3>
                <ul>${environmentData.syllabus.map(i=>`<li>${i}</li>`).join('')}</ul>
            </div>
            <div class="syllabus-section">
                <h3>Sub-themes in ${this.currentTopic.icon} ${this.currentTopic.name}</h3>
                <ul>${subthemes.map(s=>`<li>${s}</li>`).join('')}</ul>
            </div>
            <div class="syllabus-section">
                <h3>Topic Progress</h3>
                <p style="color:var(--text-muted);line-height:1.8;padding:15px;background:var(--bg);border-radius:8px;">
                    This topic has <strong style="color:var(--primary);">${total} PYQs</strong>.<br>
                    ${this.currentTopic.description}<br><br>
    ...
    renderNotes() {
        ...
        c.innerHTML = `
            <h3 style="margin-bottom:15px;color:var(--primary);">Notes for ${this.currentTopic.icon} ${this.currentTopic.name}</h3>
    ...
    renderPrelims() {
        ...
        const subthemes = [...new Set(qs.map(q => q.subtheme))];
        ...
        c.innerHTML = `
            ...
            ${subthemes.length > 1 ? `<div style="margin-bottom:18px;">
                <strong style="font-size:0.9em;">Filter by Sub-theme:</strong>
                <div class="filter-buttons" id="subthemeFilters" style="margin-top:8px;">
                    <button class="filter-btn active" data-filter="all">All</button>
                    ${subthemes.map(s => `<button class="filter-btn" data-filter="${s}">${s}</button>`).join('')}
                </div>
            </div>` : ''}
        ...
        if (subthemes.length > 1) {
            c.querySelectorAll('#subthemeFilters .filter-btn').forEach(btn => {
                btn.addEventListener('click', () => {
                    c.querySelectorAll('#subthemeFilters .filter-btn').forEach(b => b.classList.remove('active'));
                    btn.classList.add('active');
                    const filter = btn.dataset.filter;
                    c.querySelectorAll('#questionsContainer .question-card').forEach(card => {
                        card.style.display = (filter === 'all' || card.dataset.subtheme === filter) ? 'block' : 'none';
                    });
                });
            });
        }
    ...
    renderQuestion(question, index) {
        ...
        return `
            <div class="question-card ${statusClass}" data-qindex="${index}" data-subtheme="${question.subtheme || ''}">
                <div class="question-header">
                    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
                        <strong>Q${index+1}</strong>
                        ${question.subtheme ? `<span class="subtheme-tag">${question.subtheme}</span>` : ''}
                    </div>
                ...
                <div class="options">
                    ${Object.entries(question.options).map(([key,val]) => {
                        ...
                        return `<div class="option ${cls}" data-key="${key}">(${key.toUpperCase()}) ${val}</div>`;
                    }).join('')}
                </div>
                ${!attempted ? '' : `
                    <div class="answer-section" style="display:block;">
                        <div class="answer-label">${attempted.correct ? 'Correct!' : 'Incorrect'} - Answer: (${question.answer.toUpperCase()}) ${question.options[question.answer]}</div>
                ...
                ${attempted ? '' : `
                    ...
                    <div class="answer-section model-answer" data-qindex="${index}">
                        <div class="answer-label">Correct Answer: (${question.answer.toUpperCase()}) ${question.options[question.answer]}</div>
                        ${question.explanation ? `<div class="explanation-text">${question.explanation}</div>` : ''}
                    </div>`}
            </div>`;
    }
    ...
    attachQuestionHandlers(container) {
        ...
        container.querySelectorAll('.toggle-answer').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (ans.classList.contains('show')) { ans.classList.remove('show'); btn.textContent = 'Show Answer'; }
    ...
    startQuickQuiz() {
        ...
        if (unattempted.length === 0) { this.showNotification('All questions attempted!'); return; }
        ...
        c.innerHTML = `
            ...
            ${shuffled.map((q, i) => { const origIdx = qs.indexOf(q); return this.renderQuestion(q, origIdx); }).join('')}`;
        this.attachQuestionHandlers(c);
    ...
    renderTopicMains() {
        ...
        const themeMap = {
            ...
            'Pollution & Waste Management': ['Pollution & Waste'],
            'Environmental Laws & Policies': ['Environmental Law'],
            'Ecology & Ecosystems': ['Biodiversity & Conservation', 'Sustainable Development'],
            'Forests & Land Resources': ['Environmental Law', 'Sustainable Development'],
            'Water Resources & Rivers': ['Water Conservation'],
            'Sustainable Development & Renewable Energy': ['Sustainable Development'],
            'Environmental Agreements & Organizations': ['Climate Change', 'Biodiversity & Conservation'],
            'Environmental Issues & Current Affairs': ['Pollution & Waste', 'Climate Change']
        };
        const mainsThemes = themeMap[this.currentTopic.name] || [];
        const relevant = typeof environmentMainsQuestions !== 'undefined'
            ? environmentMainsQuestions.filter(q => mainsThemes.includes(q.theme))
            : [];
        ...
        c.innerHTML = `
            <div style="margin-bottom:20px;">
                ...
                <p style="color:var(--text-muted);">Questions related to ${this.currentTopic.icon} ${this.currentTopic.name}</p>
            </div>
    ...
    renderAllMains() {
        const c = document.getElementById('mainsContent');
        const allQ = typeof environmentMainsQuestions !== 'undefined' ? environmentMainsQuestions : [];
        if (allQ.length === 0) { c.innerHTML = this.emptyState('No Mains questions available'); return; }
        const years = [...new Set(allQ.map(q=>q.year))].sort((a,b)=>b-a);
        const themes = [...new Set(allQ.map(q=>q.theme))].sort();

        c.innerHTML = `
            <div style="margin-bottom:25px;">
                <h3 style="color:var(--primary);margin-bottom:15px;">All Mains Questions (${allQ.length})</h3>
                <p style="color:var(--text-muted);margin-bottom:20px;">UPSC Mains GS Paper 3 Environment & Ecology (2013-2024)</p>
                ...
                <div style="margin-bottom:15px;">
                    <h4 style="color:var(--text);margin-bottom:10px;font-size:1em;">Filter by Theme:</h4>
                    <div class="filter-buttons" id="themeFilters">
                        <button class="filter-btn active" data-filter="all">All Themes</button>
                        ${themes.map(t => `<button class="filter-btn" data-filter="${t}">${t}</button>`).join('')}
                    </div>
                </div>
            </div>
    ...
    attachMainsFilters() {
        ['yearFilters','themeFilters'].forEach(id => {
    ...
    filterMains() {
        ...
        const tf = document.querySelector('#themeFilters .filter-btn.active').dataset.filter;
        document.getElementById('mainsQuestionsContainer').innerHTML = this.renderFilteredMains(yf, tf);
    }

    renderFilteredMains(yearFilter='all', themeFilter='all') {
        let filtered = typeof environmentMainsQuestions !== 'undefined' ? environmentMainsQuestions : [];
        ...
        return `
            <div class="mains-question">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;flex-wrap:wrap;gap:6px;">
                    <div>
                        <span class="mains-year">Year: ${q.year}</span>
                        <span class="mains-marks">${q.marks} Marks</span>
                        ${q.words ? `<span class="mains-words">${q.words}W</span>` : ''}
                    </div>
                    <span class="mains-topic-tag">${q.theme}</span>
                </div>
    ...
    updateStats() {
        const totalPrelims = environmentQuestions.length;
        ...
        document.getElementById('mainsCount').textContent = typeof environmentMainsQuestions !== 'undefined' ? environmentMainsQuestions.length : 0;
...
document.addEventListener('DOMContentLoaded', () => {
    window.environmentTracker = new EnvironmentTracker();
    setTimeout(() => window.environmentTracker.selectTopic('all'), 100);
});
